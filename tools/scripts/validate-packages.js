...
function validatePackageJson(packagePath, packageType) {
  ...
  if (packageJson.name && !packageJson.name.startsWith('@dataprism/')) {
    warnings.push(`Package name "${packageJson.name}" should start with "@dataprism/"`);
  }
  ...
  Object.entries(VERSION_RULES).forEach(([dep, expectedRange]) => {
    if (allDeps[dep]) {
      ...
      if (expectedRange === 'workspace:*') {
        if (actualVersion !== 'workspace:*' && !actualVersion.startsWith('^1.')) {
          warnings.push(`${dep} should use "workspace:*" or compatible version in ${packagePath}`);
        }
      } else if (!semver.intersects(actualVersion, expectedRange)) {
        warnings.push(`${dep} version "${actualVersion}" may not be compatible with expected "${expectedRange}" in ${packagePath}`);
      }
    }
  });
  ...
  if (packageJson.exports) {
    if (typeof packageJson.exports !== 'object') {
      ...
      if (!hasDefaultExport) {
        warnings.push(`Exports should include default "." export in ${packagePath}`);
      }
    }
  }
  ...
  if (packageJson.scripts) {
    Object.entries(packageJson.scripts).forEach(([script, command]) => {
      if (typeof command === 'string' && command.includes('curl') && command.includes('sh')) {
      ...
      if (packageVersions.has(depName)) {
        ...
        if (depVersion.startsWith('^') || depVersion.startsWith('~')) {
          ...
          if (!semver.satisfies(actualVersion, depVersion)) {
            warnings.push(`Workspace dependency ${depName} version mismatch in ${pkg.name}: specified ${depVersion}, actual ${actualVersion}`);
          }
        }
      }
    });
...
function generateCompatibilityMatrix(packages) {
  ...
  const allPackages = packages.filter(p => p.packageJson && p.packageJson.name);
  ...
  commonDeps.slice(0, 10).forEach(([depName, users]) => {
    console.log(chalk.gray(`  ${depName.padEnd(30)} used by ${users.size} packages`));
  });
}
...
function main() {
  ...
  packages.forEach(pkg => {
    ...
    const { issues, warnings, packageJson } = validatePackageJson(pkg.path, pkg.type);
  ...
  if (workspaceValidation.issues.length > 0) {
    console.log(chalk.red(`  ❌ ${workspaceValidation.issues.length} workspace issue(s):`));
    workspaceValidation.issues.forEach(issue => console.log(chalk.red(`    • ${issue}`)));
    totalIssues += workspaceValidation.issues.length;
  }
  
  if (workspaceValidation.warnings.length > 0) {
    console.log(chalk.yellow(`  ⚠️  ${workspaceValidation.warnings.length} workspace warning(s):`));
    workspaceValidation.warnings.forEach(warning => console.log(chalk.yellow(`    • ${warning}`)));
    totalWarnings += workspaceValidation.warnings.length;
  }

  if (workspaceValidation.issues.length === 0 && workspaceValidation.warnings.length === 0) {
  ...
  if (totalIssues > 0 || totalWarnings > 0) {
    ...
    if (totalWarnings > 0) {
      console.log(chalk.gray('  • Consider addressing warnings for better package quality'));
      console.log(chalk.gray('  • Use consistent dependency versions across packages'));
    }
  }
