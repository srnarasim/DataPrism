...
import init, {
  QueryEngine as WasmQueryEngine,
  init_panic_hook,
} from "../../../packages/core/pkg/dataprism_core.js";
...
export class DataPrismEngine {
  ...
  private async applyWasmOptimizations(
    ...
    try {
      ...
      console.warn(
        "WASM optimization failed, falling back to original result:",
        error,
      );
      return result;
    }
...
async function runBrowserTests() {
  ...
  const initResult = await page.evaluate(async () => {
    const { DataPrismEngine } = await import(
  ...
  if (perfResult.executionTime > 2000) {
    throw new Error(
      `Performance target not met: ${perfResult.executionTime}ms > 2000ms`,
    );
  }
