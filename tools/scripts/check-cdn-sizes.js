...
function analyzeBundles() {
  ...
  files.forEach(file => {
    const sizeColor = file.size > 1024 * 1024 ? chalk.red : 
                     file.size > 512 * 1024 ? chalk.yellow : chalk.green;
    console.log(`  ${chalk.cyan(file.path.padEnd(30))} ${sizeColor(file.formattedSize.padStart(8))}`);
  });
  ...
  if (allWarnings.length > 0) {
    ...
    allWarnings.forEach(warning => {
      switch (warning.type) {
        case 'size_warning':
          console.log(chalk.yellow(`  ${warning.file} is ${warning.percentage}% of its size limit (${warning.size}/${warning.limit})`));
          break;
      }
    });
  }
  ...
  if (allIssues.length > 0) {
    ...
    allIssues.forEach(issue => {
      switch (issue.type) {
        case 'size_exceeded':
          console.log(chalk.red(`  ${issue.file} exceeds size limit by ${issue.overage} (${issue.size}/${issue.limit})`));
          break;
        case 'wasm_size_exceeded':
          console.log(chalk.red(`  WASM file ${issue.file} exceeds limit by ${issue.overage} (${issue.size}/${issue.limit})`));
          break;
        case 'total_size_exceeded':
          console.log(chalk.red(`  Total CDN size exceeds limit by ${issue.overage} (${issue.size}/${issue.limit})`));
          break;
      }
    });
  }
  ...
  if (largeFiles.length > 0) {
    console.log(chalk.gray('  • Consider code splitting for files larger than 1MB:'));
  ...
  if (wasmFiles.length > 0) {
    console.log(chalk.gray('  • WASM files should be served with streaming compilation'));
  ...
  if (allIssues.length > 0) {
    console.log('');
    console.log(chalk.red(`❌ CDN size check failed with ${allIssues.length} issue(s)`));
    process.exit(1);
  } else if (allWarnings.length > 0) {
    console.log('');
    console.log(chalk.yellow(`⚠️  CDN size check passed with ${allWarnings.length} warning(s)`));
  } else {
    console.log('');
