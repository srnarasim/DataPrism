...
const pluginCreateCommand = new Command('create')
  ...
  .option('-t, --type <type>', 'Plugin type (data-processor, visualization, integration, utility)')
  ...
  .action(async (pluginName?: string, options: {
    ...
    try {
        ...
        const namePrompt = await inquirer.prompt([
          {
            ...
            validate: (input: string) => {
              ...
              if (!validation.validForNewPackages) {
                return validation.errors?.[0] || validation.warnings?.[0] || 'Invalid plugin name';
              }
              return true;
            }
          }
      ...
      if (!nameValidation.validForNewPackages) {
        throw new Error(`Invalid plugin name: ${nameValidation.errors?.[0] || nameValidation.warnings?.[0]}`);
      }
      ...
      try {
        ...
        console.log('');
        console.log('Plugin development guide: https://docs.dataprism.dev/plugins');
