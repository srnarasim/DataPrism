...
export function DataPrismProvider({ children }: DataPrismProviderProps) {
  ...
  const [orchestrator, setOrchestrator] = useState<DataPrismOrchestrator | null>(null);
  ...
  const loadData = useCallback(async (data: any[], tableName = 'user_data') => {
    if (!engine) throw new Error('DataPrism engine not initialized');
  ...
  const getPerformanceMetrics = useCallback(async () => {
    if (!orchestrator) throw new Error('DataPrism orchestrator not initialized');
