  ...
  .action(async (options: {
    ...
    try {
      ...
      if (!(await fs.pathExists(packageJsonPath))) {
        throw new Error('package.json not found. Make sure you\'re in a DataPrism project directory.');
      }
      ...
      const conflictingDeps = [
        { name: 'webpack', reason: 'Use Vite instead for better WASM support' },
        { name: 'create-react-app', reason: 'Use dataprism init instead' }
      ];

      conflictingDeps.forEach(({ name, reason }) => {
        if (packageJson.dependencies?.[name] || packageJson.devDependencies?.[name]) {
          warnings.push(`Potentially conflicting dependency ${name}: ${reason}`);
        }
      });
      ...
      if (!(await fs.pathExists(tsConfigPath))) {
        warnings.push('No tsconfig.json found. TypeScript configuration recommended.');
      }
      ...
      for (const wasmFile of wasmFiles) {
        try {
          ...
          issues.push(`Failed to validate WASM file ${wasmFile}: ${error.message}`);
        }
      }

      // Check build configuration
      const viteConfigExists = await fs.pathExists(path.resolve(process.cwd(), 'vite.config.ts')) ||
                               await fs.pathExists(path.resolve(process.cwd(), 'vite.config.js'));
      
      if (!viteConfigExists && !packageJson.scripts?.build) {
        warnings.push('No build configuration found. Consider adding vite.config.ts or build script.');
      }
      ...
      if (options.fix && issues.length > 0) {
        ...
        if (missingDeps.length > 0) {
          const installSpinner = ora('Installing missing dependencies...').start();
      ...
      if (issues.length > 0) {
        console.log(chalk.red(`Found ${issues.length} issue(s) that need attention.`));
        if (!options.fix) {
