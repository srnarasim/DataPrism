...
export class CoreIntegration {
  ...
  private async registerCoreServices(): Promise<void> {
    ...
    this.serviceProxy.registerService("data", {
      ...
      loadData: (data: any[], tableName: string) =>
        this.coreEngine.loadData(data, tableName),
      listTables: () => this.coreEngine.listTables(),
      getTableInfo: (tableName: string) =>
        this.coreEngine.getTableInfo(tableName),
    });
...
export class ServiceProxy {
  ...
  async call(
    ...
    if (!this.hasPermission(serviceName, method)) {
      throw new Error(
        `Plugin ${this.pluginName} does not have permission to call ${serviceName}.${method}`,
      );
    }
...
describe("PluginManager", () => {
  ...
  it("should register a valid plugin manifest", async () => {
    await expect(
      pluginManager.registerPlugin(testManifest),
  ...
  it("should reject invalid plugin manifest", async () => {
    ...
    await expect(
      pluginManager.registerPlugin(invalidManifest),
    ).rejects.toThrow();
  });
  ...
  it("should execute plugin operations", async () => {
    vi.spyOn(pluginManager["loader"], "load").mockResolvedValue(mockPlugin);
    vi.spyOn(pluginManager["security"], "checkPermission").mockResolvedValue(
      undefined,
    );
  ...
  it("should enforce security permissions", async () => {
    vi.spyOn(pluginManager["loader"], "load").mockResolvedValue(mockPlugin);
    vi.spyOn(pluginManager["security"], "checkPermission").mockRejectedValue(
  ...
  it("should discover plugins automatically", async () => {
    ...
    vi.spyOn(pluginManager["loader"], "discoverPlugins").mockResolvedValue([
      "/mock/plugin/path",
    ]);
    vi.spyOn(pluginManager["loader"], "loadManifest").mockResolvedValue(
      testManifest,
    );
...
export class SampleDataProcessor implements IDataProcessorPlugin {
  ...
  getName(): string {
    return "sample-data-processor";
  }
  getVersion(): string {
    return "1.0.0";
  }
  getDescription(): string {
    return "Sample data processing plugin";
  }
  getAuthor(): string {
