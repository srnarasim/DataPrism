...
class VersionManager {
      ...
      if (packageJson[depType]) {
        Object.keys(packageJson[depType]).forEach(depName => {
          if (depName.startsWith('@dataprism/')) {
            ...
            if (currentDepVersion === 'workspace:*' || currentDepVersion.startsWith('^')) {
  ...
  generateChangelog(newVersion, changes = []) {
    ...
    if (changes.length > 0) {
      ...
      try {
        const commits = execSync(`git log --oneline --pretty=format:"%s" ${this.currentVersion}..HEAD`, { 
          encoding: 'utf8',
          cwd: rootDir 
        }).trim().split('\n').filter(Boolean);
        ...
        console.warn(chalk.yellow('Could not generate changelog from git commits'));
  ...
  async commitVersionChanges(newVersion) {
    try {
      ...
      execSync('git add package.json packages/*/package.json tools/*/package.json CHANGELOG.md', { 
        cwd: rootDir 
      });
...
Co-Authored-By: Claude <noreply@anthropic.com>"`, { 
      ...
      console.log(chalk.green(`✅ Committed version changes and created tag v${newVersion}`));
    } catch (error) {
      console.error(chalk.red('Failed to commit version changes:'), error.message);
      throw error;
    }
  }
