...
export class PerformanceMonitorPlugin implements IUtilityPlugin, ISecurityUtilityPlugin {
  ...
  async activate(): Promise<void> {
    ...
    this.context?.eventBus.subscribe('system:error', this.handleSystemError.bind(this));
    this.context?.eventBus.subscribe('plugin:*', this.handlePluginEvent.bind(this));
  }
  ...
  async getSystemStatus(): Promise<SystemStatus> {
    ...
    const status: SystemStatus = {
      timestamp: new Date(),
      overall: this.determineOverallHealth(memoryUsage, cpuUsage, responseTime, errorRate),
      components: {
        memory: {
          status: memoryUsage.percentUsed > this.thresholds.memory ? 'warning' : 'healthy',
        ...
        performance: {
          status: responseTime > this.thresholds.responseTime ? 'warning' : 'healthy',
          responseTime,
          details: { averageResponseTime: responseTime }
        },
        errors: {
          status: errorRate > this.thresholds.errorRate ? 'critical' : 'healthy',
          errorRate,
  ...
  async performHealthCheck(): Promise<HealthStatus> {
    ...
    const results = checks.map((check, index) => ({
      name: ['core-services', 'data-integrity', 'network', 'disk-space', 'plugins'][index],
    ...
    const healthStatus: HealthStatus = {
      overall: results.every(r => r.status === 'passed') ? 'healthy' : 'unhealthy',
      checks: results,
      score: (results.filter(r => r.status === 'passed').length / results.length) * 100,
      timestamp: new Date(),
    ...
    this.log('info', `Health check completed: ${healthStatus.overall} (${healthStatus.score}% passed)`);
    return healthStatus;
  }
  ...
  async performSecurityScan(options?: any): Promise<SecurityScan> {
    ...
    const securityScan: SecurityScan = {
      ...
      status: findings.some(f => f.severity === 'critical') ? 'critical' : 
              findings.some(f => f.severity === 'high') ? 'high' : 'clean',
      findings,
  ...
  async getVulnerabilityReport(): Promise<VulnerabilityReport> {
    ...
    const vulnerabilities = allFindings.filter(f => f.type === 'vulnerability');
  ...
  async optimizePerformance(options?: any): Promise<any> {
    ...
    this.log('info', `Performance optimization completed: ${optimizations.length} optimizations applied`);
    return result;
  }
  ...
  async cleanupOldData(): Promise<any> {
    ...
    for (const [metricName, values] of this.metrics) {
      const filteredValues = values.filter(v => new Date(v.timestamp) > cutoffDate);
    ...
    // Clean up old alerts
    const oldAlerts = this.alerts.filter(a => new Date(a.timestamp) <= cutoffDate);
    ...
    this.log('info', `Cleanup completed: removed ${removedCount} metrics, ${oldAlerts.length} alerts`);
    return result;
  }
  ...
  private initializeMetrics(): void {
    const metricTypes = [
      'systemStatus', 'healthCheck', 'securityScan', 'memoryUsage', 
      'cpuUsage', 'responseTime', 'errorRate', 'alerts'
    ];
  ...
  private async checkAlerts(): Promise<void> {
    ...
    if (systemStatus.components.memory.usage > this.thresholds.memory) {
      this.createAlert('memory', 'high', `Memory usage at ${systemStatus.components.memory.usage}%`);
    }
    
    // Check CPU threshold
    if (systemStatus.components.cpu.usage > this.thresholds.cpu) {
      this.createAlert('cpu', 'high', `CPU usage at ${systemStatus.components.cpu.usage}%`);
    }
    
    // Check response time threshold
    if (systemStatus.components.performance.responseTime > this.thresholds.responseTime) {
      this.createAlert('performance', 'medium', `Response time at ${systemStatus.components.performance.responseTime}ms`);
    }
    
    // Check error rate threshold
    if (systemStatus.components.errors.errorRate > this.thresholds.errorRate) {
      this.createAlert('errors', 'critical', `Error rate at ${systemStatus.components.errors.errorRate}%`);
    }
  }
  ...
  private determineOverallHealth(memory: any, cpu: number, responseTime: number, errorRate: number): string {
    if (errorRate > this.thresholds.errorRate) return 'critical';
    if (memory.percentUsed > this.thresholds.memory || cpu > this.thresholds.cpu) return 'warning';
  ...
  private generateSecurityRecommendations(findings: any[]): string[] {
    ...
    if (critical.length > 0) {
      recommendations.push(`Address ${critical.length} critical security findings immediately`);
    }
    
    const high = findings.filter(f => f.severity === 'high');
    if (high.length > 0) {
      recommendations.push(`Review ${high.length} high-severity security issues`);
    }
  ...
  private generateVulnerabilityRecommendations(vulnerabilities: any[]): string[] {
    ...
    if (vulnerabilities.length === 0) {
      ...
      recommendations.push('Prioritize patching based on severity and exploitability');
  ...
  private generateOptimizationRecommendations(optimizations: any[]): string[] {
    ...
    if (optimizations.length > 0) {
      ...
      recommendations.push('Consider additional optimizations if issues persist');
    } else {
  ...
  private async optimizeMemory(): Promise<any> {
    ...
    return { type: 'memory', description: 'Cleared unused caches', impact: '15% reduction' };
  }

  private async optimizeCPU(): Promise<any> {
    // Mock CPU optimization
    return { type: 'cpu', description: 'Optimized processing algorithms', impact: '10% reduction' };
  }

  private async optimizeCache(): Promise<any> {
    // Mock cache optimization
    return { type: 'cache', description: 'Optimized cache policies', impact: '20% hit rate improvement' };
  }

  private async optimizePlugins(): Promise<any> {
    // Mock plugin optimization
    return { type: 'plugins', description: 'Deactivated unused plugins', impact: '5% resource reduction' };
  }
...
export const manifest: PluginManifest = {
  ...
  version: '1.0.0',
  description: 'Comprehensive performance monitoring and security scanning utility plugin',
